...
import {
    GameConfigProvider,
    useGameConfig,
} from '../provider/GameConfigProvider';
...
function GameConfigViewInner(props: {
    ...
    const availableGameModes = (): ReactNode[] => {
        return Object.values(GameMode)
            ...
            .map(mode => (
                <Picker.Item
                    label={gameModeAsString(mode as GameMode)}
                    value={mode}
                    key={mode}
                />
            ));
    ...
    return (
        <Fullscreen backgroundColor={theme.colors.background}>
                ...
                <ScrollView>
                    <View
                        ...
                        <ConfigRow text="Spielmodus" fixedWidth={200}>
                            <Picker
                                placeholder="Modus"
                                selectedValue={gameConfig.gameMode}
            ...
            <View
                ...
                <Button
                    title="Starten"
                    onPress={() => props.onStartGame(gameConfig)}
                    horizontalCentered
                />
            </View>
        </Fullscreen>
...
const TeamConfig = (props: { team: 'teamA' | 'teamB' }) => {
    const [gameConfig, setGameConfig] = useGameConfig();
    const [teamName, setTeamName] = useState(
        gameConfig.getTeamName(props.team),
    );
