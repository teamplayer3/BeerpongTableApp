...
export function PlayerSelector(props: {
    ...
    const onPressAddButton = () => {
        if (!addPlayer) {
            ...
            if (newPlayerName.length !== 0) {
                ...
                console.log(
                    'new player with id ' + id + ' and name ' + newPlayerName,
                );
    ...
    const isPlayerFiltered = (player: Player): boolean => {
        ...
        return !isUndefined(
            props.filteredPlayers.find(p => p.id === player.id),
        );
    };
    ...
    return (
        <Fullscreen backgroundColor={theme.colors.background}>
            <HeaderBar
                headerLabel="Spieler Auswahl"
                onClose={props.onClosePlayerSelection}
            />
            <SectionList
                sections={playerStore.getGroupedByStartLetter().map(g => ({
                    title: g.letter,
                    data: g.players,
                }))}
