...
import React, { Dispatch, ReactElement, ReactNode, SetStateAction, useRef, useState } from "react"
import { useEffect } from "react"
import { Animated, Easing, LayoutAnimation, LayoutChangeEvent, Pressable, StyleSheet, Text, View } from "react-native"
...
export default function Collapsable(props: {
    ...
    return (
        <CollapsableContext.Provider value={[expanding, setExpanding]}>
            {React.cloneElement(props.children.header, { backgroundColor: props.backgroundColor })}
            {React.cloneElement(props.children.body, { backgroundColor: props.backgroundColor })}
...
export const Header = (props: {
    ...
    const arrowIcon = (isUndefined(expanding) || !expanding) ? "arrow-drop-down" : "arrow-drop-up"
    const onToggling = () => {
...
export const Body = (props: {
    ...
    const [expanding, _] = React.useContext(CollapsableContext)
    ...
    const expandAnimation = useRef(new Animated.Value(isUndefined(expanding) || expanding ? 0 : 1)).current
    ...
    return (
        <Animated.View
            ...
            style={{
                overflow: "hidden",
                height: isUndefined(expandedHight) ? undefined : expandAnimation.interpolate({
                    inputRange: [0, 1],
                    outputRange: [0, expandedHightDefined + style.body.marginTop]
                })
            }}
        >
            <React.Fragment>
            ...
            <View style={{ ...style.body, backgroundColor: props.backgroundColor, position: "absolute", width: "100%", opacity: isUndefined(expandedHight) ? 0 : 1 }} onLayout={onLayout}>
                {props.children}
            </View>
