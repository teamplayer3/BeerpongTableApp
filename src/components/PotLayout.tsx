...
export function PotLayout(props: {
    ...
    const styledPotComp = (idx: number) => {
        const pot = props.pots.find(tPot => tPot.potId === idx)!;
        return (
            <PotComponent
                key={idx}
                pressable={pot.pressable}
                bordered={pot.style.bordered}
                color={pot.style.color}
                overlay={pot.style.overlay}
                potId={idx}
                // TODO: correct type
                size={
                    Animated.multiply(POT_SIZE, scale) as Animated.Value as any
                }
                onPress={() => onPress(idx)}
            />
        );
    ...
    const row = (startId: number, potCount: number) => {
        let pots = Array.from({ length: potCount }, (_, i) => startId + i).map(
            (idx, _) => styledPotComp(idx),
        );
        return (
    ...
    return (
        <View
            ...
            {React.Children.map(children, child => (
                <Animated.View
                    style={{ marginVertical: Animated.divide(gap, 2) }}>
                    {child}
                </Animated.View>
            ))}
        </View>
