...
import { LayoutChangeEvent, LayoutRectangle, PanResponder, PanResponderGestureState, View, ViewStyle } from "react-native";
...
export default function ColorPicker(props: {
    ...
    const colorToX = (width: number, color: Color) => {
        const max = 0xFF
        const step = (step: number) => 1 / 6 * step * width
        const interpolate = (step_: number, c: number) => step(step_) + ((step(step_ + 1) - step(step_)) / (max - 0)) * (c - 0)
    ...
    const calcInterpolatedColor = (x: number) => {
        const max = 0xFF
        const step = (step: number) => 1 / 6 * step * rect.width
        const interpolate = (step_: number, x: number) => 0 + ((max - 0) / (step(step_ + 1) - step(step_))) * (x - step(step_))
        let r = 0, g = 0, b = 0
            ...
            >
                <Defs>
                    <LinearGradient id="gradient">
                        <Stop offset={`${100 / 6 * 0}%`} stopColor="#ff0000" />
                        <Stop offset={`${100 / 6 * 1}%`} stopColor="#ffff00" />
                        <Stop offset={`${100 / 6 * 2}%`} stopColor="#00ff00" />
                        <Stop offset={`${100 / 6 * 3}%`} stopColor="#00ffff" />
                        <Stop offset={`${100 / 6 * 4}%`} stopColor="#0000ff" />
                        <Stop offset={`${100 / 6 * 5}%`} stopColor="#ff00ff" />
                        <Stop offset={`${100 / 6 * 6}%`} stopColor="#ff0000" />
                    </LinearGradient>
                </Defs>

                <Rect x="0" y="0" width="100%" height={height} fill="url(#gradient)" />
            </Svg>
