...
import { scanBarcodes, BarcodeFormat, Barcode, BarcodeValueType } from 'vision-camera-code-scanner';
...
export default function QRCodeScanner(props: {
    ...
    const onQRCodeDetected = useCallback((qrCode: Barcode) => {
        if (qrCode.content.type === BarcodeValueType.TEXT && validateQrCode(qrCode.content.data)) {
            props.onFoundCorrectQRCode(qrCode)
        }
    }, [])
    ...
    return (
        (!isUndefined(device) && hasPermission) ?
            <MemCamera
                key="camera-comp"
                style={{
