...
export class GameState {
    ...
    shotPot = (pot: SpecificPot, _shotType: ShotType) => {
        if (!isUndefined(this.specialEvent)) {
            this.specialEvent.selectPot(pot);
    ...
    reorderNotShotPots = (teamId: number, notShotIndexes: number[]) => {
        ...
        this.getTeam(teamId)?.pots.forEach(pot => {
            ...
            pot.state.shotCount =
                inNewPattern === undefined ? NEEDED_SHOTS_PER_POT : 0;
        });
    };
    ...
    getPot = (pot: SpecificPot) => {
        return this.teams
            .find(value => value.id === pot.teamId)
    ...
    teamHasLost = () => {
        let team = this.teams.find(searchTeam =>
            searchTeam.pots.every(value => isPotFullyShot(value)),
        );
        if (team !== undefined) {
    ...
    remainingPots = (teamId: number) => {
        return this.teams
            .find(value => value.id === teamId)!
            .pots.reduce(
                (prev, curr) => (isPotFullyShot(curr) ? prev - 1 : prev),
                10,
            );
    };
    ...
    createGameStatistics = (): GameStatistics | undefined => {
        ...
        return new GameStatistics(
            ...
            this.teams.map(team => {
                return {
                    teamId: team.id,
                    remainingCups: team.pots.reduce(
                        (prev, curr) => (prev += isPotFullyShot(curr) ? 0 : 1),
                        0,
                    ),
...
export class NormalMode extends SpecialEvent {
    ...
    private evaluateNextEvent = () => {
        const potObj = !isUndefined(this.lastHitPot)
            ? this.gameState.getPot(this.lastHitPot)
            : undefined;
        const manyHits = this.hitPots >= this.gameState.gameConfig.ballsPerTeam;
        const shotWasABomb =
            !isUndefined(potObj) &&
            isPotFullyShot(potObj) &&
            potObj.state.shotsPerRound > 1;
        const teamHasNoShotsOver =
            this.gameState.currTeamShotCount >=
            this.gameState.gameConfig.ballsPerTeam;

        if (shotWasABomb) {
            this.computedNextEvent = new BombShot(
                this.gameState,
                this.lastHitPot!,
            );
        } else if (manyHits) {
...
export class BombShot extends SpecialEvent {
    ...
    isOver(): boolean {
        return (
            this.selectedPots ===
            this.gameState.gameConfig.extraConfig.ballsToSelectForBomb
        );
    }
