...
class TableConnection {
    ...
    private receiveListeners: Map<number, ReceiveCallback> = new Map<
        number,
        ReceiveCallback
    >();

    private static stateListenerCount: number = 0;
    private stateListeners: Map<number, StateCallback> = new Map<
        number,
        StateCallback
    >();
    ...
    private startBluetooth = async () => {
        const bluetoothIsOff =
            (await this.bleManager.state()) === State.PoweredOff;
        if (bluetoothIsOff) {
    ...
    private registerBleStateListener = () => {
        ...
        this.bleStateListener = this.bleManager.onStateChange(
    ...
    private onBleStateChange = (newState: State) => {
        switch (newState) {
            case State.PoweredOn:
                this.discoverTableConnection();
                break;
            case State.PoweredOff:
                Alert.alert('Enable Bluetooth to use all functions');
                break;
        ...
        try {
            if (!this.bleDevice || !(await this.bleDevice.isConnected())) {
                ...
                if (await this.bleManager.isDeviceConnected(tableIdent)) {
                    device = (
                        await this.bleManager.connectedDevices([tableIdent])
                    )[0];
                } else {
    ...
    private registerDeviceLostConnectionListener = () => {
        ...
        if (this.deviceLostConnectionListener)
            this.deviceLostConnectionListener.remove();

        this.deviceLostConnectionListener = this.bleDevice.onDisconnected(
            this.onDeviceDisconnected,
        );
    };

    private onDeviceDisconnected = async (
        _err: BleError | null,
        _device: Device | null,
    ) => {
    ...
    private registerDeviceCharacteristicListener = (): void => {
        ...
        this.deviceCharacteristicListener =
            this.bleDevice.monitorCharacteristicForService(
                serviceUUID,
                characteristicUUID,
                this.onReceiveBytesIntern,
            );
    };

    private onReceiveBytesIntern = (
        err: BleError | null,
    ...
    private disconnectDevice = async () => {
        if (this.deviceLostConnectionListener)
            this.deviceLostConnectionListener.remove();
        if (this.deviceCharacteristicListener)
            this.deviceCharacteristicListener.remove();
        if (this.bleDevice && (await this.bleDevice!.isConnected()))
